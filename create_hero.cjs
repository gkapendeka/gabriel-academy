...
const heroContent = `import React, { useEffect, useRef } from 'react';
...
.side-chip {
  ...
  gap: 8px;
  opacity: 0;
  animation: fadeChip 1s ease forwards;
}
body.light-mode .side-chip {
  background: rgba(0,0,0,0.04);
  border: 0.5px solid rgba(0,0,0,0.1);
}
.side-chip:nth-child(1) { animation-delay: 0.4s; }
.side-chip:nth-child(2) { animation-delay: 0.7s; }
.side-chip:nth-child(3) { animation-delay: 1.0s; }
@keyframes fadeChip {
  to { opacity: 1; }
}
.chip-dot {
...
.chip-dot.purple { background: #a78bfa; box-shadow: 0 0 6px #a78bfa80; }
.chip-dot.blue   { background: #60a5fa; box-shadow: 0 0 6px #60a5fa80; }
.chip-dot.teal   { background: #34d399; box-shadow: 0 0 6px #34d39980; }
.chip-dot.amber  { background: #fbbf24; box-shadow: 0 0 6px #fbbf2480; }
.chip-dot.pink   { background: #f472b6; box-shadow: 0 0 6px #f472b680; }
.chip-dot.coral  { background: #fb923c; box-shadow: 0 0 6px #fb923c80; }
.chip-label { font-size: 11px; color: var(--muted); white-space: nowrap; font-weight: 400; letter-spacing: 0.01em; }
.chip-value { font-size: 12px; color: var(--text); white-space: nowrap; font-weight: 500; margin-left: auto; }
\`;

fs.writeFileSync('src/components/HeroAnimation.jsx', heroContent, 'utf8');
fs.writeFileSync('src/components/HeroAnimation.css', cssContent, 'utf8');

// Now update Landing.jsx to use HeroAnimation
let landingContent = fs.readFileSync('src/pages/Landing.jsx', 'utf8');
...
const newHero = \`{/* Hero */}
      <HeroAnimation>
        <div style={{zIndex: 1, padding: '100px 24px'}}>\`;

landingContent = landingContent.replace(oldHero, newHero);

// Close the HeroAnimation tag
const oldHeroClose = \`          <div style={{display: 'flex', gap: '24px', justifyContent: 'center', flexWrap: 'wrap', fontSize: '14px', color: 'var(--muted)', fontWeight: 500}}>
...
const newHeroClose = \`          <div style={{display: 'flex', gap: '24px', justifyContent: 'center', flexWrap: 'wrap', fontSize: '14px', color: 'var(--muted)', fontWeight: 500}}>
            <span>✓ 100% Confidential</span>
            <span>✓ Verified Experts Only</span>
            <span>✓ Quality Guaranteed</span>
            <span>✓ On-Time Delivery</span>
...
landingContent = landingContent.replace(oldHeroClose, newHeroClose);

fs.writeFileSync('src/pages/Landing.jsx', landingContent, 'utf8');
console.log('Successfully created HeroAnimation and integrated into Landing.jsx');
